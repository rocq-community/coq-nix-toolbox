...
You can create directories named after a Coq package and containing `default.nix` files in `.nix/rocq-overlays` to override the contents of `rocqPackages`.
...
Jobs represent buildable outputs. You can build any package in `rocqPackages` (including any package defined in your `.nix/rocq-overlays` directory) with the following command:
...
- `--arg override '{p1 = v1; ...; pn = vn;}'`: a very condensed inline way to select specific versions of `rocq-core` or any package from `rocqPackages` (using `--arg coq-override`) or `ocamlPackages` (using `--arg ocaml-override`). E.g. `--arg override '{rocq-core = "9.0"; ...; mathcomp = "2.3.0";}'` to override the current default bundle with the given versions.
